...
public class BoardDaoImpl implements BoardDao{
	...
	public List<Board> selectList(Map<String, Object> paramMap) {
		 ...
		 * 
		 * 2. OFFEST FETCH를 사용하여 쿼리 조회(오라클 12이상에서 사용가능)
		 ...
		 *  ...조건절
		 *  ORDER BY 절
		 *  OFFEST 시작행 ROWS FETCH NEXT 조회할 개수 ROWS ONLY
		 * 3. RowBounds를 활용한 방식
		 *  - MyBatis에서 쿼리 결과에 대해 페이징 처리를 적용해주는 도구
		 *  - 전체 쿼리결과를 자바어플리케이션으로 가져온 후, 지정한 위치(offset)에서 
		 *    특정 개수(limit)를 잘라내는 방식으로 페이징 처리를 진행한다.
		 *  - 오라클의 offset fetch문법과 비슷하며, 어플리케이션으로 가져올 데이터가 수만건 이상인 경우, 심각한 메모리낭비 및 성능저하가 발생할 수 있다.
