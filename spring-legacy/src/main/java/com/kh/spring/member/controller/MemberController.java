...
public class MemberController {
	 ...
	 *  - new 연산자를 직접 사용하지 않고, Autowired 어노테이션을 통해 주입받는다.
	 */
...
//	}
	 ...
	 * */
//	public String login(@RequestParam(value = "userId", defaultValue = "jbj") String userId) {
...
//		/*
//		 * 응답데이터를 담을 수 잇는 객체. (model)
//		 * 1) Model
//		 *  - 포워딩할 응갑 뷰체이지에 전달하고자 하는 데이터를 맵형식을호 담을 수 있는 객체.
//		 *  - 기본적으로 request scope를 가지고 있으며, 설정을 통해 session scope에도 데이터를 담을 수 있다.
//		 *  - 클래스 선언부에 @SessionAttributes를 추가하면 데이터가 session scope에 저장된다. 
//		 * 
//		 * 2) ModelAndView
//		 *  - ModelandView에서 Model은 Model데이터를 의미한다.
//		 *  - VIEW는 이동할 페이지에 대한 정보를 담고있는 객체를 의미한다.
...
//		
//		// Jsckson-databind를 활용하여, VO클래스, 컬렉션 데이터를 자동으로 JSON으로 반환해줄 수 있다.
//		return searchMember;
//	}
	 ...
	 * 	- 컨트롤러에 예외처리가 메서드를 1개 추가한 후, @ExcoptionHandler어노테이션추가
	 *  - 2순위로 적용
	 * 3. 어플리케이션 전역에서 발생하는 예외를 모아서 처리하는 방법
	 *  - 새로운 클래스 작성후 ControllerAdvice어노테이션 추가
	 *  - 3순위
	 */
