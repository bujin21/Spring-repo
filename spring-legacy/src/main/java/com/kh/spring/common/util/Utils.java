...
public class Utils {	
	 ...
	 * XSS(크로스 사이트 스크립트공격)
	 *  - 스크립트 삽입 공격
	 *  - 사용자가 script태그를 게시글에 작성하여 클라이언트가 
	 *  게시글 클릭시 script에 지정한 코드가 실행되도록 유도하는 방식
	 *  - 위 내용을 그대로 db에 저장 후 브아우저에 랜더링하면 문제가 발생할 수 있으므로 
	 *  태그가 아닌 기본 문자열로 인식할 수 있게씀 html내부 entity로 변환처리를 수행해야한다.
	 * 
	 */
